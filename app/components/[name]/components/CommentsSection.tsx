...
export function CommentsSection({ componentId }: CommentsSectionProps) {
  ...
  function buildTree(comments: Comment[]): Record<number, Comment[]> {
  ...
  function renderNested(
    comment: Comment,
    byParent: Record<number, Comment[]>,
    isReply = false,
    parentUser?: string
  ) {
  ...
  const fetchComments = async (append = false) => {
    try {
      ...
      const { data } = await axios.get(`/api/comments?${params.toString()}`);
      if (append) {
  ...
  const handleReply = async (parentId: number) => {
    ...
    try {
      ...
      const updateCommentId = (comment: Comment) => ({
        ...comment,
        id: data.id,
      });
  ...
  const handleReaction = async (
    ...
    try {
      ...
      if (current === type) {
        // Remove reaction
        await axios.delete("/api/comments/reaction", { data: { commentId } });
        updatedReactions = (
          allComments.find((c) => c.id === commentId)?.reactions ?? []
        ).filter((r) => r.userId !== userId);
      } else {
        // Add or update reaction
        await axios.post("/api/comments/reaction", { commentId, type });
