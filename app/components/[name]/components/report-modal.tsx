...
export function ReportModal({ componentId, hasPurchased }: ReportModalProps): JSX.Element {
  ...
  async function onSubmit(values: z.infer<typeof formSchema>) {
    if (!session?.user) {
      toast.error("Please sign in to report a component.");
      return;
    }

    if (!canReport) {
      toast.error("You have been blocked from reporting components.");
      setOpen(false);
      return;
    }
    ...
    try {
      ...
      const response = await fetch("/api/reportc", {
        method: "POST",
        body: formData,
      });
      ...
      <DialogContent className="overflow-y-scroll scrollbar-hide bg-zinc-100 dark:bg-zinc-900 sm:max-w-[425px] max-h-[90vh]">
        <DialogHeader className="space-y-3">
          ...
          <DialogDescription className="text-sm sm:text-base">
            {!hasPurchased ? (
              "You must purchase this component before you can report it."
            ) : !canReport ? (
              "You have been blocked from reporting components."
            ) : (
              "If you think a component has stolen your work or has other questionable content, please use the form below to report it."
            )}
          </DialogDescription>
        </DialogHeader>
        {!hasPurchased ? (
          <div className="p-4 bg-amber-50 dark:bg-amber-950 border border-amber-200 dark:border-amber-800 rounded-md">
            <p className="text-amber-800 dark:text-amber-200 text-sm">
              You need to purchase this component before you can report issues
              with it. This ensures reports come from actual users of the
              product.
            </p>
          </div>
        ) : !canReport ? (
          <div className="p-4 bg-red-50 dark:bg-red-950 border border-red-200 dark:border-red-800 rounded-md">
            <p className="text-red-800 dark:text-red-200 text-sm">
              You have been blocked from reporting components. Please contact support if you believe this is an error.
            </p>
          </div>
        ) : (
          <Form {...form}>
                      ...
                      <div
                        className={`flex items-center justify-center w-full py-6 border-2 border-dashed rounded-md cursor-pointer transition-colors relative ${
                          isDragging
                            ...
                            : "border-gray-300 dark:border-zinc-700 hover:border-gray-400 dark:hover:border-gray-600 bg-gray-50 dark:bg-zinc-800 hover:bg-gray-100 dark:hover:bg-zinc-700"
                        }`}
                        onDragOver={handleDragOver}
                        onDragLeave={handleDragLeave}
                        onDrop={handleDrop}
                        onClick={() => !previewUrl && document.getElementById('file-upload-input')?.click()}
                      >
                        {previewUrl ? (
