...
const formSchema = z.object({
  ...
  categoriesInput: z.string().min(1, 'At least one category is required').optional(),
  pagesListInput: z.string().min(1, 'At least one page is required').optional(),
...
export default function EditTemplatePage({ params, searchParams }: Props) {
  ...
  useEffect(() => {
    const fetchTemplate = async () => {
      try {
        ...
        reset({
          ...template,
          categoriesInput: template.categories.join(', '),
          pagesListInput: template.pagesList.join(', '),
          features: template.features.length ? template.features : [{ question: '', answer: '' }],
          images: template.images.length ? template.images : [{ src: '', alt: '' }],
        });
      } catch (error) {
        console.error('Error fetching template:', error);
        toast.error('Failed to load template data');
        router.push('/admin/edittemplates');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const onSubmit = async (data: FormValues) => {
    try {
      ...
      const { categoriesInput, pagesListInput, ...rest } = data;
      const apiData = {
        ...rest,
        categories,
        pagesList,
      ...
      console.log('Submitting data to:', `/api/template/${params.id}`);
  ...
  return (
    <div className="container py-10 bg-background">
      <div className="flex items-center gap-4 mb-6">
        <Button variant="ghost" onClick={() => router.push('/templates')} className="p-0 h-auto">
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="newtemplate" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            <Button type="button" disabled={isSubmitting} 
              onClick={() => {
                ...
                if (Object.keys(form.formState.errors).length > 0) {
                  console.log('Detailed errors:', JSON.stringify(form.formState.errors));
                }
