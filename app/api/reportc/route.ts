...
export async function POST(req: Request) {
  ...
  try {
    ...
    const componentId = data.get("componentId") as string; // componentId is now a string
    const userId = Number(data.get("userId"));
    const file = data.get("image") as File | null;

    if (!name || !email || !reason || !details || !componentId || !userId) {
    ...
    const report = await prisma.reportComponent.create({
      data: {
        name,
        email,
        reason,
        details,
        componentId, // componentId is now a string, no need to parse
        userId,
