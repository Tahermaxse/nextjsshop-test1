...
export async function POST(request: Request) {
  try {
    ...
    return NextResponse.json({ message: 'Template added successfully', template }, { status: 201 });
  } catch (error) {
    console.error('Error adding template:', error);
    return NextResponse.json({ message: 'Failed to add template', error }, { status: 500 });
  } finally {
