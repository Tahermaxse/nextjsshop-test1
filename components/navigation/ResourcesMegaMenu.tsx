  ...
  return (
    <>
      ...
      <div className="overflow-hidden rounded-lg shadow-lg ring-1 ring-black ring-opacity-5">
        <div className="grid w-full grid-cols-[0.9fr,0.55fr,0.55fr] divide-x divide-neutral-200 dark:divide-white/20 bg-white dark:bg-zinc-900">
          {/* Explore Column */}
          <div className="flex h-full flex-col p-4">
            ...
            <div className="grid grow grid-cols-2 gap-4">
              {exploreItems.map((item, index) => (
                <Link
                  key={index}
                  className="group relative isolate z-0 flex flex-col justify-between overflow-hidden rounded-xl border border-neutral-100 bg-neutral-100 px-5 py-4 transition-colors duration-75 dark:border-white/20 dark:bg-zinc-800"
                  href={item.href}
                >
                  <div className="absolute inset-0 opacity-0 transition-opacity duration-150 group-hover:opacity-100">
                    <div className="absolute -inset-[25%] -skew-y-12 [mask-image:linear-gradient(225deg,black,transparent_50%)]">
                      <svg
                        className="pointer-events-none absolute inset-0 translate-y-2 text-[#22c55e88] transition-transform duration-150 ease-out group-hover:translate-y-0"
                        width="100%"
                        height="100%"
                      >
                        <defs>
                          <pattern
                            id={`grid-${item.title.toLowerCase().replace(/\s+/g, '-')}`}
                            x={-1}
                            y={-15}
                            width={46}
                            height={46}
                        ...
                        <rect fill={`url(#grid-${item.title.toLowerCase().replace(/\s+/g, '-')})`} width="100%" height="100%" />
                      </svg>
                    </div>
                    <div className="absolute -inset-[10%] opacity-10 blur-[50px] dark:brightness-150 bg-[conic-gradient(#22c55e_0deg,#22c55e_117deg,#16a34a_180deg,#15803d_240deg,#22c55e_360deg)]" />
                  </div>
                  {item.icon}
          ...
          <div className="px-6 py-4">
            <p className="text-xs uppercase text-neutral-500 dark:text-white/60 mb-2">
              Company
            </p>
            <div className="flex flex-col gap-0.5">
              {companyItems.map((item, index) => (
                <Link
                  key={index}
                  className="group rounded-[8px] p-2 transition-colors hover:bg-neutral-100 active:bg-neutral-100 dark:hover:bg-zinc-800 dark:active:bg-zinc-700 -mx-2"
                  href={item.href}
                >
                  <div className="flex items-center justify-between gap-3">
                    <div className="shrink-0 rounded-md border border-neutral-200 bg-white/50 p-2.5 dark:border-zinc-700 dark:bg-zinc-800">
                      {item.icon}
                    </div>
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-neutral-500 dark:text-white/60 line-clamp-1">
                        {item.description}
                      </p>
                    </div>
                  </div>
